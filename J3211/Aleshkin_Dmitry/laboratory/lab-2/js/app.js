...
const CATEGORY_OPTIONS_HTML = Api.CATEGORIES.map((item) => `<option value="${item.value}">${item.label}</option>`).join('');
  ...
  container.innerHTML = `
    <div class="demo-banner">
      ...
      <div class="d-flex gap-2 flex-wrap">
        ...
        <a class="btn btn-outline-primary" href="register.html">Регистрация</a>
      </div>
    </div>
  `;
}
...
async function renderDashboardPage(context) {
  ...
  document.getElementById('budgetTable').innerHTML = budgets.length
    ...
    : '<tr><td colspan="5" class="text-center text-muted">Бюджеты пока не заданы</td></tr>';
...
function bindBudgetControls(state) {
  ...
  if (state.isDemo) {
    ...
    showPageNotice('budgetNotice', 'В демо-режиме бюджеты доступны только для просмотра.', 'info');
    return;
  }
...
function renderExpenseChart(transactions) {
  ...
  expenseChart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{ data: grouped.map((item) => item.amount), borderWidth: 1 }],
    },
    options: {
      responsive: true,
  ...
  document.getElementById('transactionTable').innerHTML = filtered.length
    ? filtered.map((item) => `
        <tr>
          ...
          <td class="${toNumber(item.amount) > 0 ? 'text-success' : 'text-danger'}">${Api.formatCurrency(item.amount)}</td>
          <td>
