...
const auth = useAuth()
const { currentUser, loadCurrentUser, requireAuth, updateCurrentUser } = auth
...
const handleCommentSubmit = async () => {
  ...
  const nextComments = isEditing
    ? course.value.comments.map((comment) =>
      comment.userId === user.id ? nextComment : comment,
    )
    : [...course.value.comments, nextComment]

  try {
    course.value = await api.updateCourse(course.value.id, {
...
const handleStartLearning = async () => {
  ...
  if (user.learningCourseIds.includes(course.value.id)) {
  ...
  try {
    const updatedUser = await api.updateUser(user.id, {
      learningCourseIds: [...user.learningCourseIds, course.value.id],
    })
