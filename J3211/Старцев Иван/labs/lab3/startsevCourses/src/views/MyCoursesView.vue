...
const getMyCourses = () => {
  ...
  return courses.value.filter((course) => currentUser.value.createdCourseIds.includes(course.id))
}

const myCourses = computed(() => getMyCourses())
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditing) {
      ...
      await api.updateCourse(editingCourseId.value, {
        ...payload,
      ...
      const updatedUser = await api.updateUser(currentUser.value.id, {
