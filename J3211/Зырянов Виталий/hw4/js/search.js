(function () {
  ...
  function renderActiveChips(filters) {
    ...
    chips.forEach(function (chipText) {
      const span = document.createElement("span");
      span.className = "chip";
      span.setAttribute("aria-label", `Активный фильтр: ${chipText}`);
        ...
        <div class="d-flex justify-content-between flex-wrap gap-2">
          <div class="result-head">
            <span class="result-avatar" aria-hidden="true">
              <svg class="icon">
                <use href="../sprites/sprite.svg#icon-avatar-model"></use>
              </svg>
            </span>
            <h3 class="h5 mb-1"><a href="model.html?id=${model.id}" aria-label="Открыть страницу модели ${model.title}">${model.title}</a></h3>
          </div>
          <span class="status-badge ${statusClass}" aria-label="Статус модели: ${statusLabel}">Model ${statusLabel}</span>
        </div>
