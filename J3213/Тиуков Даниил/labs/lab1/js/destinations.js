document.addEventListener("DOMContentLoaded", () => {
    ...
    function bindEvents() {
        ...
        page.addEventListener("click", (event) => {
            ...
            if (saveButton) {
                ...
                const favorites = TravelApp.addArrayItem(
                    TravelApp.storageKeys.favorites,
                    destinationId,
                    (entry, nextItem) => Number(entry) === Number(nextItem)
                );
                updateSaveButtons(favorites);
    ...
    function getFilteredDestinations() {
        ...
        const filtered = TravelAppData.destinations.filter((destination) => {
                ...
                <article class="destination-card card border-0">
                    ...
                    <div class="card-body d-flex flex-column">
                        <div class="card-meta mt-0">
                            ...
                            <span class="badge badge-soft">${TravelApp.escapeHtml(destination.duration)} дн.</span>
                        </div>
    ...
    function updateSaveButtons(favorites) {
        document.querySelectorAll("[data-save-destination]").forEach((button) => {
            const isFavorite = favorites.some((id) => Number(id) === Number(button.dataset.saveDestination));
