...
export const useFinanceStore = defineStore('finance', {
  ...
  actions: {
    ...
    async importTransactions(userId) {
      ...
      const missingTransactions = importBatch.filter((item) => {
        return !this.transactions.some((existing) => (
          String(existing.userId) === String(userId)
          && String(existing.date) === String(item.date)
          && String(existing.category) === String(item.category)
          && String(existing.description) === String(item.description)
          && String(existing.type) === String(item.type)
          && normalizeTransactionAmount(existing.amount) === normalizeTransactionAmount(item.amount)
          && String(existing.importSource || '') === String(item.importSource || '')
        ))
      })

      if (!missingTransactions.length) {
        return 'Импорт уже выполнен ранее. Новых транзакций для загрузки нет.'
      }

      const importTotal = missingTransactions.reduce((sum, item) => sum + normalizeTransactionAmount(item.amount), 0)
      ...
      await Promise.all(missingTransactions.map((item) => financeApi.createTransaction(item)))
      await this.loadAll(userId)

      if (missingTransactions.length === importBatch.length) {
        return 'Импорт выполнен. Загружены демонстрационные транзакции из платёжного аккаунта.'
      }

      return `Импорт выполнен частично. Добавлено новых транзакций: ${missingTransactions.length}.`
    },
