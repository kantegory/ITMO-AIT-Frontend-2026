      ...
      <div id="navMenu" class="collapse navbar-collapse">
        <ul class="navbar-nav ms-auto gap-lg-2 mt-3 mt-lg-0 align-items-lg-center">
          ...
          <li class="nav-item theme-switch-item">
            <button class="theme-toggle-btn" type="button" @click="uiStore.toggleTheme()">
              <span class="theme-toggle-icon">
                <IconSprite :name="uiStore.theme === 'dark' ? 'icon-moon' : 'icon-sun'" aria-hidden />
              </span>
              <span>{{ uiStore.theme === 'dark' ? 'Тёмная тема' : 'Светлая тема' }}</span>
            </button>
          </li>
          <li v-if="authStore.isAuthenticated" class="nav-item">
...
function handleLogout() {
  authStore.logout()
  uiStore.showToast('Вы вышли из аккаунта')
  router.push('/login')
}
</script>
