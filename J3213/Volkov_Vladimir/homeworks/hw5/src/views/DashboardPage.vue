        ...
        <div class="page-card h-100">
          ...
          <form class="row g-3" @submit.prevent="submitTransaction">
            ...
            <div class="col-md-6">
              ...
              <input id="txDate" v-model="transactionForm.date" class="form-control rounded-4" type="date" required />
            </div>
            <div class="col-12">
      ...
      <div class="col-lg-6">
        <div class="page-card h-100">
          ...
          <div class="quick-grid">
            ...
            <div class="metric-list-item">
              <div class="metric-label">Доходов</div>
              <div class="metric-value">
                {{ incomeTransactions.length }}
              </div>
            </div>
            <div class="metric-list-item">
              <div class="metric-label">Расходов</div>
              <div class="metric-value">
...
function applyFilters() {
  uiStore.showToast('Фильтр применён')
}
...
function resetFilters() {
  ...
  filters.amount = null
  filters.date = ''
  uiStore.showToast('Фильтры сброшены')
}
...
async function handleOperation(type) {
  ...
  if (!amount || amount <= 0) {
    uiStore.showToast('Введите сумму больше нуля')
    return
  }
...
async function submitTransaction() {
  ...
  if (!normalizedAmountValue) {
    uiStore.showToast('Сумма должна быть больше нуля')
    return
  }

  if (Number(transactionForm.amount) < 0) {
    uiStore.showToast('Нельзя вводить сумму с минусом. Укажите положительное значение.')
    return
  }
  ...
  try {
    ...
    uiStore.showToast('Транзакция добавлена')
  } catch (error) {
    uiStore.showToast(error.message || 'Ошибка при добавлении транзакции')
  }
}
...
async function removeTransaction(item) {
  ...
  try {
    ...
    uiStore.showToast('Транзакция удалена')
  } catch (error) {
    uiStore.showToast(error.message || 'Не удалось удалить транзакцию')
  }
}
...
onMounted(async () => {
  await Promise.allSettled([
    financeStore.loadAll(authStore.user.id),
    marketsStore.refreshAll()
  ])
})
</script>
