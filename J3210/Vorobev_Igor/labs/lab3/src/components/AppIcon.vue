<template>
  <svg
    ...
    focusable="false"
  >
    <path v-if="name === 'sun'" :d="paths.sun" />
    <path v-else-if="name === 'moon'" :d="paths.moon" />
    <path v-else-if="name === 'search'" :d="paths.search" />
  </svg>
</template>
...
const props = defineProps({
  name: {
    type: String,
    required: true,
    validator: (value) => ['sun', 'moon', 'search'].includes(value),
  },
  size: {
    type: [Number, String],
    default: 20,
...
const viewBox = computed(() => (props.name === 'moon' ? '0 0 24 24' : '0 0 16 16'))
</script>
