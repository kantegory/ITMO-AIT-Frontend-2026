...
import { addComment, findDataset, reactToItem, store } from "../store.js";

const route = useRoute();
const tab = ref("description");
const text = ref("");
const dataset = computed(() => findDataset(route.params.id));
const source = computed(() => store.myDatasets.some(item => item.id === Number(route.params.id)) ? "myDatasets" : "globalDatasets");
const comments = computed(() => store.comments.filter(comment => comment.page === "dataset" && comment.resourceId === Number(route.params.id)));
