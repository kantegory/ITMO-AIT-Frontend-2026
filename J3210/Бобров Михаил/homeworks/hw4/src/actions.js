...
export function exposeActions() {
    ...
    window.deleteSubscriptionFromDetails = deleteSubscriptionFromDetails;
}
...
async function deleteSubscriptionFromDetails() {
    await deleteItem("subs", currentId());
    if (!state.mySubscriptions.some(item => item.id === currentId())) {
