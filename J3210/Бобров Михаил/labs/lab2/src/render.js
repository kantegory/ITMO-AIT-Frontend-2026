            ...
            <div class="col-md-6 col-lg-4">
                <div class="card card-item p-3 h-100 border-0 shadow-sm">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-auto">
                        ...
                        <button class="btn btn-primary btn-sm" type="button" onclick="addToMy('models', ${model.id})">Добавить</button>
                    </div>
                </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card card-item p-3 h-100 border-0 shadow-sm">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-auto">
                        ...
                        <button class="btn btn-primary btn-sm" type="button" onclick="addToMy('datasets', ${dataset.id})">Добавить</button>
                    </div>
                </div>
...
export function renderModels() {
    ...
    const html = state.myModels.map(model => dash ? `
        ...
        <tr>
            ...
            <td class="text-end pe-4"><button class="btn btn-light btn-sm text-danger" type="button" onclick="deleteItem('models', ${model.id})">Удалить</button></td>
        </tr>`).join("");
...
export function renderDatasets() {
    ...
    const html = state.myDatasets.map(dataset => dash ? `
        ...
        <tr>
            ...
            <td class="text-end pe-4"><button class="btn btn-light btn-sm text-danger" type="button" onclick="deleteItem('datasets', ${dataset.id})">Удалить</button></td>
        </tr>`).join("");
    if (dash) dash.innerHTML = html;
    if (table) table.innerHTML = html;
}
...
export function renderSubscriptions() {
    ...
    const html = state.mySubscriptions.map(subscription => dash ? `
        ...
        <tr>
            ...
            <td class="text-end pe-4"><button class="btn btn-light btn-sm text-danger" type="button" onclick="deleteItem('subs', ${subscription.id})">Отписаться</button></td>
        </tr>`).join("");
    if (dash) dash.innerHTML = html;
    if (table) table.innerHTML = html;
}
