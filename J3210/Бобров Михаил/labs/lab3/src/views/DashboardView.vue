    ...
    <div class="content-area">
        ...
        <div class="row g-4" role="tabpanel" aria-live="polite">
            <template v-if="tab === 'models'">
                <div v-for="model in resources.myModels" :key="model.id" class="col-md-4"><ResourceCard :item="model" kind="models" compact /></div>
            </template>
            <template v-else-if="tab === 'datasets'">
                <div v-for="dataset in resources.myDatasets" :key="dataset.id" class="col-md-4"><ResourceCard :item="dataset" kind="datasets" compact /></div>
            </template>
            <template v-else>
                <div v-for="subscription in resources.mySubscriptions" :key="subscription.id" class="col-md-4"><ResourceCard :item="subscription" kind="subscriptions" compact /></div>
            </template>
        </div>
    </div>
