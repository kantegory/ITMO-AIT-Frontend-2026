...
const { addToCollection, resources } = useResources();
const models = computed(() => resources.globalModels.filter(item => matches(item, true)));
const datasets = computed(() => resources.globalDatasets.filter(item => matches(item, false)));
