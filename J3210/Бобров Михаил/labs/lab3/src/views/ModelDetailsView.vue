...
const text = ref("");
const { addComment, findModel, reactToItem, resources } = useResources();
const model = computed(() => findModel(route.params.id));
const source = computed(() => resources.myModels.some(item => item.id === Number(route.params.id)) ? "myModels" : "globalModels");
const comments = computed(() => resources.comments.filter(comment => comment.page === "model" && comment.resourceId === Number(route.params.id)));
