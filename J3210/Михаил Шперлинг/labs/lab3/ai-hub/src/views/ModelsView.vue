...
const { search, activeTasks, selectedFramework, filtered } = useFilters(items)
...
<template>
  <base-layout>
    <div class="row">
      <aside class="col-md-3 mb-4" aria-label="Фильтры">
        <filter-panel
          ...
          v-model:selected-framework="selectedFramework"
        />
      </aside>

      <main class="col-md-9" aria-label="Список моделей">
        ...
        <div v-else class="row row-cols-1 row-cols-md-2 g-4" aria-live="polite">
          <div v-for="item in filtered" :key="item.id" class="col">
            <resource-card :item="item" type="models" />
          </div>
    ...
    <create-resource-modal ref="createModal" @created="loadItems" />
  </base-layout>
</template>
