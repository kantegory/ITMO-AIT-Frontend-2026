function renderEventCard(event, headingTag = 'h2') {
  ...
  const metaLine = `${event.dateShort}${event.timeLabel ? ', ' + event.timeLabel : ''} · ${event.cityLabel}, ${event.venueShort}`;
  ...
  col.innerHTML = `
    <article class="card event-card h-100">
      ...
      <div class="card-body event-card__body">
        ...
        <p class="event-card__meta mb-0">${metaLine}</p>
  ...
  // Название мероприятия задаёт организатор, поэтому оно попадает на страницу
  // как текст, а не как часть HTML-строки.
...
// Любой запрос к API проходит три состояния: загрузка, успех, ошибка.
// Пока идёт запрос, показываем индикатор; сетку и сообщение прячем.
async function loadEvents(filters = {}, limit = null) {
  ...
  try {
    ...
    // на главной карточки идут под заголовком «Ближайшие мероприятия», поэтому там h3
