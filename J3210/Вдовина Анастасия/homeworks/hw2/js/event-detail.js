...
// Имя и текст отзыва вводит пользователь, поэтому карточка собирается через
// createElement и textContent: подстановка такой строки в HTML означала бы, что
// введённая разметка выполнится как код страницы.
...
function renderSidebar(event) {
  ...
  if (event.free) {
    // Некоторым событиям нужен свой вопрос участнику (например, вид транспорта
    // на велопрогулке). В ЛР1 такой блок был вписан прямо в разметку страницы,
    // теперь он приходит из данных и рисуется, только если поле заполнено.
    ...
    sidebar.innerHTML = `
      ...
      ${optionsBlock}
      <button type="button" class="btn btn-brand w-100 js-open-action">Записаться</button>
    `;
  } else {
    sidebar.innerHTML = `
      ...
      <button type="button" class="btn btn-brand w-100 js-open-action">Купить билет</button>
    `;
...
function buildModalContent(event) {
  ...
  // название мероприятия задаёт организатор — подставляем его текстом
...
function initReviewForm(event) {
  ...
  // Отзыв сохраняется в записи мероприятия, а изменять записи разрешено
  // только вошедшим пользователям — значит, гостю форму показывать незачем.
