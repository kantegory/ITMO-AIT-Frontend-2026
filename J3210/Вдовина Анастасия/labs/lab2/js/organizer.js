...
async function loadOrganizerEvents() {
  ...
  let refunds;
  try {
    // три независимых запроса выгоднее отправить параллельно, а не один за другим
...
function initCreateEventForm() {
  ...
  button.addEventListener('click', async () => {
    ...
    // id попадает в адрес страницы (event.html?id=...), поэтому оставляем в нём
    // только латиницу и цифры; для названия целиком на кириллице берём слово event
    const slug = title
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
