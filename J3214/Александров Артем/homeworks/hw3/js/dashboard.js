...
async function loadAndRenderTransactions(userId) {
    ...
    if (!transactions || transactions.length === 0) {
        container.innerHTML = `<div class="theme-text-muted small text-center py-3">Транзакций пока нет</div>`;
        return;
    }
    ...
    container.innerHTML = transactions.map(tx => {
        ...
        const amountClass = isIncome ? 'text-success' : 'theme-text-main';
        ...
        <div class="d-flex justify-content-between align-items-center pb-2">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <div class="fw-semibold theme-text-main" style="font-size: 14px;">${tx.title}</div>
                    <div class="theme-theme-text-muted" style="font-size: 11px;">${tx.category}</div>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <button class="btn btn-sm btn-outline-danger border-0 p-1" aria-label="Удалить" onclick="handleDeleteTx('${tx.id}', '${userId}')">
