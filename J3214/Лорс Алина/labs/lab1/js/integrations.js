(() => {
  ...
  function renderIntegrations() {
    ...
    document.getElementById('integrationsGrid').innerHTML = items.map((item) => `<div class="col-12 col-md-6 col-xl-4"><article class="integration-card"><div class="integration-card__top"><div class="d-flex gap-3"><div class="integration-card__icon"><i class="bi bi-${item.icon}"></i></div><div><div class="integration-card__title">${item.service}</div><p class="integration-card__meta">${item.description}</p>${window.TarelkaUI.statusMarkup(item.connected ? 'connected' : 'disconnected')}</div></div></div><p class="small text-secondary mb-3">Последняя синхронизация: ${formatDateTime(item.lastSync)}</p><div class="d-flex gap-2 flex-wrap"><button class="btn btn-sm btn-outline-primary" data-connect-id="${item.id}">${item.connected ? 'Настроить' : 'Подключить'}</button><button class="btn btn-sm btn-outline-secondary" data-toggle-id="${item.id}">${item.connected ? 'Отключить' : 'Включить'}</button></div></article></div>`).join('');
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (toggleBtn) { const integration = window.TarelkaStorage.getIntegrations().find((item) => item.id === toggleBtn.dataset.toggleId); if (!integration) return; window.TarelkaStorage.toggleIntegration(integration.id, !integration.connected); renderIntegrations(); window.TarelkaUI.showToast(integration.connected ? `Сервис ${integration.service} отключён.` : `Сервис ${integration.service} активирован.`); }
      const deleteRuleBtn = event.target.closest('[data-rule-delete]');
      if (deleteRuleBtn) { window.TarelkaStorage.deleteImportRule(deleteRuleBtn.dataset.ruleDelete); renderRules(); window.TarelkaUI.showToast('Правило импорта удалено.'); }
    });
    ...
    document.getElementById('integrationForm').addEventListener('submit', (event) => {
      ...
      window.TarelkaStorage.toggleIntegration(id, true, { tokenPreview: '••••••set', lastSync: syncValue }); integrationModal.hide(); renderIntegrations(); window.TarelkaUI.showToast('Подключение сервиса сохранено.');
    });
    ...
    document.getElementById('ruleForm').addEventListener('submit', (event) => {
      ...
      window.TarelkaStorage.addImportRule(payload); document.getElementById('ruleForm').reset(); document.getElementById('ruleKeywordGroup').classList.remove('d-none'); document.getElementById('ruleAmountGroup').classList.add('d-none'); ruleModal.hide(); renderRules(); window.TarelkaUI.showToast('Новое правило импорта добавлено.');
    });
    ...
    document.getElementById('importForm').addEventListener('submit', async (event) => {
      ...
      const imported = window.TarelkaStorage.importTransactions(parseCsv(await file.text()), accountId, format, file.name); renderImportHistory(); window.TarelkaUI.showToast(`Импортировано ${imported.length} записей.`);
    });

    document.getElementById('useSampleImportBtn').addEventListener('click', () => {
      ...
      const imported = window.TarelkaStorage.importTransactions(parseCsv(window.TarelkaData.sampleImportCsv), accountId, 'csv', 'sample-transactions.csv'); renderImportHistory(); window.TarelkaUI.showToast(`Тестовый CSV обработан: ${imported.length} записей добавлено.`);
    });
  }

  document.addEventListener('DOMContentLoaded', () => {
    if (document.body.dataset.page !== 'integrations') return;
    window.TarelkaStorage.requireAuth();
