(() => {
  ...
  function renderMetrics(data) {
    ...
    document.getElementById('reportsInsights').innerHTML = insights.map((item) => `<article class="insight-card"><div class="insight-card__title">${item.title}</div><p class="insight-card__text">${item.text}</p></article>`).join('');
  }
  ...
  function bindPeriodButtons() { document.querySelectorAll('[data-period]').forEach((button) => button.addEventListener('click', () => { state.period = button.dataset.period; document.querySelectorAll('[data-period]').forEach((item) => item.classList.toggle('active', item === button)); renderAll(); })); }
  document.addEventListener('DOMContentLoaded', () => { if (document.body.dataset.page !== 'reports') return; window.TarelkaStorage.requireAuth(); bindPeriodButtons(); renderAll(); });
})();
