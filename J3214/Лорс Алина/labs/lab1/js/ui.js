window.TarelkaUI = (() => {
  ...
  function createSidebarMarkup(currentPage, user) {
    const links = navItems.map((item) => `<a class="sidebar-link ${currentPage === item.page ? 'active' : ''}" href="${item.href}"><i class="bi bi-${item.icon}"></i><span>${item.label}</span></a>`).join('');
    return `
      ...
      <div class="sidebar-footer"><p>Данные и настройки сохраняются локально, без сервера.</p><button class="btn btn-light w-100" id="logoutBtnSidebar">Выйти</button></div>
    `;
  }
  ...
  function renderLayout() {
    ...
    const user = window.TarelkaStorage.requireAuth();
    if (!user) return;
    ...
    if (topbar) {
      topbar.innerHTML = `<header class="topbar"><nav class="navbar navbar-expand-lg px-3 px-lg-4"><button class="btn btn-outline-secondary d-lg-none me-2" type="button" data-bs-toggle="offcanvas" data-bs-target="#mobileSidebar"><i class="bi bi-list fs-4"></i></button><div class="topbar-search me-auto"><i class="bi bi-search"></i><input class="form-control" type="search" id="globalSearchField" placeholder="Быстрый поиск по интерфейсу"></div><div class="topbar-meta ms-lg-3"><span class="topbar-notice d-none d-md-inline-flex"><i class="bi bi-lock me-2"></i>Frontend-only demo</span><div class="user-chip"><div class="user-chip__avatar">${getInitials(user.firstName, user.lastName)}</div><div><div class="fw-semibold">${user.firstName} ${user.lastName}</div><div class="topbar-footer-note">${user.email}</div></div></div></div></nav></header>`;
    }
    const footer = document.getElementById('footerContainer');
    if (footer) { footer.className = 'app-footer'; footer.innerHTML = `<div class="d-flex justify-content-between flex-wrap gap-2"><span>Tarelka · клиентский прототип управления финансами</span></div>`; }
    document.addEventListener('click', (event) => { if (event.target.closest('#logoutBtnSidebar')) window.TarelkaStorage.logout(); });
    const globalSearchField = document.getElementById('globalSearchField');
    if (globalSearchField) globalSearchField.addEventListener('keydown', (event) => { if (event.key !== 'Enter') return; const value = event.currentTarget.value.trim().toLowerCase(); const map = { кабинет: 'dashboard.html', транзакции: 'transactions.html', отчёты: 'reports.html', интеграции: 'integrations.html' }; const match = Object.keys(map).find((key) => value.includes(key)); if (match) window.location.href = map[match]; });
  }

  function showToast(message, variant = 'primary') {
    const container = document.getElementById('toastContainer'); if (!container) return;
    const toastElement = document.createElement('div'); toastElement.className = 'toast align-items-center text-bg-light border-0'; toastElement.role = 'alert';
    ...
    const toast = new bootstrap.Toast(toastElement, { delay: 2600 }); toast.show(); toastElement.addEventListener('hidden.bs.toast', () => toastElement.remove());
  }
  ...
  function statusMarkup(status) { const map = { completed: ['completed', 'Завершена'], pending: ['pending', 'В обработке'], connected: ['connected', 'Подключён'], disconnected: ['disconnected', 'Не подключён'], normal: ['normal', 'Норма'], warning: ['warning', 'Почти лимит'], exceeded: ['exceeded', 'Превышен'] }; const [className, label] = map[status] || ['normal', status]; return `<span class="status-badge status-badge--${className}">${label}</span>`; }

  return { renderLayout, showToast, formatCurrency, formatDate, fillSelect, typeLabel, statusMarkup };
})();

document.addEventListener('DOMContentLoaded', () => { window.TarelkaUI.renderLayout(); });
