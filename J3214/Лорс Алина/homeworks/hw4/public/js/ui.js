window.TarelkaUI = (() => {
  ...
  function iconMarkup(iconName, className = '') {
    const classes = ['sprite-icon', className].filter(Boolean).join(' ');
    return `<svg class="${classes}" aria-hidden="true" focusable="false"><use href="assets/icons.svg#icon-${iconName}"></use></svg>`;
  }

  function themeIconMarkup(iconName) {
    return iconMarkup(iconName, 'theme-icon');
  }
  ...
  function createSidebarMarkup(currentPage, user) {
    const links = navItems
      .map((item) => `<a class="sidebar-link ${currentPage === item.page ? 'active' : ''}" href="${item.href}" ${currentPage === item.page ? 'aria-current="page"' : ''}>${iconMarkup(item.icon)}<span>${item.label}</span></a>`)
      .join('');

    return `
      <div class="sidebar-brand">
        <div class="sidebar-brand__icon" aria-hidden="true">${iconMarkup('pie-chart-fill')}</div>
  ...
  return { renderLayout, showToast, formatCurrency, formatDate, fillSelect, typeLabel, statusMarkup, escapeHtml, applyTheme, getCurrentTheme, initAccessibility, iconMarkup };
})();
