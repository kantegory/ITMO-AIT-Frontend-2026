(() => {
  ...
  function renderExchangeRates() {
    const target = document.getElementById('exchangeRatesRow');
    if (!target) return;
    const cached = window.TarelkaStorage.getExchangeRatesCache();
    if (!cached || !Array.isArray(cached.rates) || !cached.rates.length) {
      target.innerHTML = '<div class="col-12"><div class="card shadow-sm border-0"><div class="card-body"><div class="text-secondary">Курсы валют загрузятся после обращения к внешнему API.</div></div></div></div>';
      return;
    }

    const cards = cached.rates.map((item) => `<div class="col-12 col-md-6 col-xl-4"><div class="card shadow-sm border-0 h-100"><div class="card-body"><div class="d-flex justify-content-between align-items-start gap-3"><div><div class="metric-card__title">Курс ${item.base}/${item.quote}</div><div class="metric-card__value">${Number(item.rate).toFixed(4)}</div><p class="metric-card__meta">Источник: Frankfurter API · дата ${item.date}</p></div><div class="metric-card__icon">${window.TarelkaUI.iconMarkup('currency-exchange')}</div></div></div></div></div>`).join('');
    target.innerHTML = cards;
  }

  async function loadExchangeRates() {
    try {
      await window.TarelkaStorage.fetchExchangeRates(getCurrency());
      renderExchangeRates();
    } catch (error) {
      renderExchangeRates();
      window.TarelkaUI.showToast(error.message, 'warning');
    }
  }
  ...
  function renderSummary() {
    ...
    document.getElementById('dashboardTitle').textContent = `Добро пожаловать, ${currentUser.firstName}`;
    ...
    document.getElementById('accountsGrid').innerHTML = metrics.accounts.map((account) => `<div class="col-12 col-md-6"><article class="account-card" aria-label="Счёт ${window.TarelkaUI.escapeHtml(account.name)}"><div class="account-card__head"><div><div class="account-card__title">${account.name}</div><p class="account-card__meta">${window.TarelkaData.accountTypes[account.type] || account.type}</p></div><div class="account-card__icon">${window.TarelkaUI.iconMarkup(typeIcon(account.type))}</div></div><div class="account-card__balance">${window.TarelkaUI.formatCurrency(account.currentBalance, account.currency)}</div><div class="d-flex justify-content-between align-items-center gap-3 flex-wrap"><span class="summary-chip">${window.TarelkaUI.iconMarkup('cash-stack')}${account.currency}</span></div></article></div>`).join('');
    ...
    document.getElementById('budgetsList').innerHTML = metrics.budgets.map((budget) => {
      ...
      return `<article class="budget-card" aria-label="Бюджет ${window.TarelkaUI.escapeHtml(budget.category)}"><div class="budget-card__head"><div><h3 class="h6 mb-1">${budget.category}</h3>${window.TarelkaUI.statusMarkup(status)}</div><button class="btn btn-sm btn-outline-primary edit-budget-btn" data-budget-id="${budget.id}" aria-label="Редактировать бюджет ${window.TarelkaUI.escapeHtml(budget.category)}">Редактировать</button></div><div class="budget-card__values"><div><span class="budget-card__value-label">Лимит</span><div class="budget-card__value">${window.TarelkaUI.formatCurrency(budget.limit, currency)}</div></div><div><span class="budget-card__value-label">Потрачено</span><div class="budget-card__value">${window.TarelkaUI.formatCurrency(budget.spent, currency)}</div></div><div><span class="budget-card__value-label">Остаток</span><div class="budget-card__value">${window.TarelkaUI.formatCurrency(Math.max(budget.limit - budget.spent, 0), currency)}</div></div></div><div class="progress mb-2" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.min(100, percentage)}" aria-label="Использовано ${percentage}% бюджета ${window.TarelkaUI.escapeHtml(budget.category)}"><div class="progress-bar ${barClass}" style="width:${Math.min(100, percentage)}%"></div></div><small class="text-secondary">${percentage}% от месячного лимита</small></article>`;
    }).join('');
  }
  ...
  function initModals() {
    ...
    document.getElementById('transactionForm').addEventListener('submit', async (event) => {
      ...
      try {
        await window.TarelkaStorage.addTransaction(payload); transactionModal.hide(); renderSummary(); syncTransactionFormOptions(); window.TarelkaUI.showToast('Транзакция успешно сохранена.');
    ...
    document.getElementById('accountForm').addEventListener('submit', async (event) => {
      ...
      try {
        await window.TarelkaStorage.addAccount(payload); document.getElementById('accountForm').reset(); accountModal.hide(); renderSummary(); syncTransactionFormOptions(); window.TarelkaUI.showToast('Новый счёт добавлен.');
      } catch (error) { window.TarelkaUI.showToast(error.message, 'danger'); }
    });
    ...
    document.getElementById('budgetForm').addEventListener('submit', async (event) => {
      ...
      try {
        await window.TarelkaStorage.updateBudget(id, limit); budgetModal.hide(); renderSummary(); window.TarelkaUI.showToast('Бюджет обновлён.');
      } catch (error) { window.TarelkaUI.showToast(error.message, 'danger'); }
    });
    ...
    document.getElementById('transferForm').addEventListener('submit', async (event) => {
      ...
      try {
        await window.TarelkaStorage.addTransfer(payload); document.getElementById('transferForm').reset(); transferModal.hide(); renderSummary(); syncTransactionFormOptions(); window.TarelkaUI.showToast('Перевод между счетами выполнен.');
      } catch (error) { window.TarelkaUI.showToast(error.message, 'danger'); }
    });
  }
