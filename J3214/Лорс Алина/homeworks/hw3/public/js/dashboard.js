(() => {
  ...
  function renderSummary() {
    ...
    document.getElementById('accountsGrid').innerHTML = metrics.accounts.map((account) => `<div class="col-12 col-md-6"><article class="account-card" aria-label="Счёт ${window.TarelkaUI.escapeHtml(account.name)}"><div class="account-card__head"><div><div class="account-card__title">${account.name}</div><p class="account-card__meta">${window.TarelkaData.accountTypes[account.type] || account.type}</p></div><div class="account-card__icon"><i class="bi bi-${typeIcon(account.type)}" aria-hidden="true"></i></div></div><div class="account-card__balance">${window.TarelkaUI.formatCurrency(account.currentBalance, account.currency)}</div><div class="d-flex justify-content-between align-items-center gap-3 flex-wrap"><span class="summary-chip"><i class="bi bi-cash-stack" aria-hidden="true"></i>${account.currency}</span></div></article></div>`).join('');
