...
function togglePassword(id, btn) {
  ...
  btn.setAttribute('aria-label', s ? 'Скрыть пароль' : 'Показать пароль');
...
function initStarRating(id) {
  ...
  const stars = [...box.querySelectorAll('[data-star]')]; let sel = 0;
  const hl = u => stars.forEach((s,i) => {
    s.textContent = i <= u ? '★' : '☆';
    s.style.color = i <= u ? '#f59e0b' : '#d1d5db';
  });
  const setRating = idx => {
    sel = idx + 1;
    hl(idx);
    box.dataset.rating = sel;
    stars.forEach((s,i) => s.setAttribute('aria-checked', i === idx ? 'true' : 'false'));
  };
  stars.forEach((s, i) => {
    s.addEventListener('mouseover', () => hl(i));
    s.addEventListener('mouseout',  () => hl(sel - 1));
    s.addEventListener('click',     () => setRating(i));
    s.addEventListener('keydown',   e => {
      if (e.key === 'ArrowRight' && i < stars.length - 1) { e.preventDefault(); stars[i+1].focus(); setRating(i+1); }
      if (e.key === 'ArrowLeft'  && i > 0)                { e.preventDefault(); stars[i-1].focus(); setRating(i-1); }
    });
  });
}
...
function initChips() {
  document.querySelectorAll('[data-chips-group]').forEach(g => {
    ...
    g.querySelectorAll('.filter-chip').forEach(c => c.addEventListener('click', () => {
      if (!multi) {
        g.querySelectorAll('.filter-chip').forEach(x => {
          x.classList.remove('active','selected');
          x.setAttribute('aria-pressed', 'false');
        });
      }
      const isNowActive = !c.classList.contains('active');
      c.classList.toggle('active'); c.classList.toggle('selected');
      c.setAttribute('aria-pressed', isNowActive ? 'true' : 'false');
    }));
  });
}
...
function fillUserUI() {
  ...
  document.querySelectorAll('.nav-avatar').forEach(el => {
    ...
    el.setAttribute('aria-label', `Меню пользователя: ${fullName}`);
  });
  const nameEl = document.getElementById('userName');
  if (nameEl) nameEl.textContent = fullName;
  const initEl = document.getElementById('userInitial');
  if (initEl) { initEl.textContent = initials; initEl.setAttribute('aria-label', `Аватар пользователя ${fullName}`); }
}
...
function renderNotes(notes) {
  ...
  list.innerHTML = notes.map(n => `
    <div class="note-item" role="article" aria-label="Заметка: ${esc(n.title)}">
      <div class="d-flex justify-content-between align-items-start mb-1">
        <strong style="font-size:.875rem;">✏️ ${esc(n.title)}</strong>
        <div class="d-flex align-items-center gap-2">
          <span style="font-size:.75rem;color:var(--text-light);" aria-hidden="true">${n.date || ''}</span>
          <button onclick="deleteNoteAPI(${n.id})" aria-label="Удалить заметку «${esc(n.title)}»" style="background:none;border:none;color:var(--text-light);cursor:pointer;padding:0;font-size:.9rem;line-height:1;"><i class="bi bi-x" aria-hidden="true"></i></button>
        </div>
      </div>
...
function initSearchPage() {
  ...
  sl?.addEventListener('input', () => {
    filters.maxBudget = +sl.value;
    const formatted = Number(sl.value).toLocaleString('ru') + ' ₽';
    if (ve) ve.textContent = formatted;
    sl.setAttribute('aria-valuenow', sl.value);
    sl.setAttribute('aria-valuetext', formatted);
    renderDestinations();
  });
  document.querySelectorAll('[data-type-chip]').forEach(c => c.addEventListener('click', () => {
    document.querySelectorAll('[data-type-chip]').forEach(x => { x.classList.remove('active','selected'); x.setAttribute('aria-pressed','false'); });
    c.classList.add('active','selected'); c.setAttribute('aria-pressed','true'); filters.type = c.dataset.typeChip; renderDestinations();
  }));
      ...
      <div class="dest-card" onclick="location.href='destination.html?id=${d.id}'">
        ...
        <div class="dest-card-body">
          ...
          <div class="d-flex gap-2 mt-1">
            ...
            <button class="${sv ? 'btn-primary-custom' : 'btn-outline-custom'}" style="font-size:.78rem;padding:.35rem .6rem;" onclick="event.stopPropagation();toggleSave(this,'${d.id}','${d.name}')" aria-label="${sv ? 'Убрать из сохранённых: ' : 'Сохранить: '}${d.name}"><i class="bi bi-bookmark${sv ? '-fill' : ''}" aria-hidden="true"></i></button>
          </div>
        </div>
      </div>
    </div>`;
...
function toggleSave(btn, id, name) {
  ...
  if (saved.includes(id)) {
    saved = saved.filter(x => x !== id); btn.className = 'btn-outline-custom';
    btn.innerHTML = '<i class="bi bi-bookmark" aria-hidden="true"></i>';
    btn.setAttribute('aria-label', `Сохранить ${name}`);
    showToast(`${name} убран`, 'info'); API.unsaveRoute(id).catch(() => {});
  } else {
    saved.push(id); btn.className = 'btn-primary-custom';
    btn.innerHTML = '<i class="bi bi-bookmark-fill" aria-hidden="true"></i>';
    btn.setAttribute('aria-label', `Убрать ${name} из сохранённых`);
