...
async function initSearchPage() {
  ...
  try {
    ...
    console.error(error);
    resultsEl.innerHTML = `<div class="empty-state">Не удалось загрузить направления с API.</div>`;
  }
}
...
async function initDestinationPage() {
  ...
  try {
    ...
    g1.alt = `${place.title} фото 1`;
    g2.alt = `${place.title} фото 2`;
    g3.alt = `${place.title} фото 3`;
