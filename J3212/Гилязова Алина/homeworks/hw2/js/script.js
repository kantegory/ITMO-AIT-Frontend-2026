...
function renderAuthButtons() {
    ...
    if (!user) {
        container.innerHTML = `
            <a href="login.html" class="btn btn-outline-light me-2" aria-label="Перейти на страницу входа">Вход</a>
    ...
    container.innerHTML = `
        <a href="profile.html" class="btn btn-outline-light me-2" aria-label="Открыть личный кабинет пользователя ${escapeHtml(user.name)}">${escapeHtml(user.name)}</a>
