...
function getIconMarkup(iconName, className = "ui-icon") {
    return `<svg class="${className}" aria-hidden="true" focusable="false"><use href="${ICON_SPRITE_PATH}#${iconName}" xlink:href="${ICON_SPRITE_PATH}#${iconName}"></use></svg>`;
}
...
function updateThemeIcon() {
    const iconUse = document.querySelector(".theme-switcher svg use");
    if (!iconUse) return;

    const icon = getThemeIcon();
    iconUse.setAttribute("href", `${ICON_SPRITE_PATH}#${icon}`);
    iconUse.setAttribute("xlink:href", `${ICON_SPRITE_PATH}#${icon}`);
}
...
function renderAuthButtons() {
    ...
    if (!user) {
        container.innerHTML = `
            ${themeSwitcher}
            <a href="login.html" class="btn btn-outline-light" aria-label="Перейти на страницу входа"><span class="btn-icon">${getIconMarkup("login")}<span>Вход</span></span></a>
            <a href="register.html" class="btn btn-warning" aria-label="Перейти на страницу регистрации"><span class="btn-icon">${getIconMarkup("user")}<span>Регистрация</span></span></a>
        `;
        applyTheme();
        return;
    }
    ...
    container.innerHTML = `
        ${themeSwitcher}
        <a href="profile.html" class="btn btn-outline-light" aria-label="Открыть личный кабинет пользователя ${escapeHtml(user.name)}"><span class="btn-icon">${getIconMarkup("user")}<span>${escapeHtml(user.name)}</span></span></a>
        <button type="button" class="btn btn-danger" onclick="logoutUser()" aria-label="Выйти из аккаунта"><span class="btn-icon">${getIconMarkup("login")}<span>Выйти</span></span></button>
    `;
    applyTheme();
}
        ...
        <div class="col-md-4 event-card"
            ...
            <article class="card h-100 event-list-card" aria-labelledby="event-title-${escapeHtml(event.id)}">
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <p class="meta-with-icon mb-3" aria-label="Город и дата">${getIconMarkup("calendar")}<span>${metaText}</span></p>
                    <a href="event.html?id=${encodeURIComponent(event.id)}" class="btn btn-primary mt-auto" aria-label="Подробнее о мероприятии ${safeTitle}"><span class="btn-icon">${getIconMarkup("ticket")}<span>Подробнее</span></span></a>
                </div>
            </article>
        </div>
    `;
        ...
        if (!events.length) {
            container.innerHTML = `
                <div class="card p-4 border-0 shadow-sm">
                    ...
                    <p class="mb-0 text-muted"><span class="inline-icon-text">${getIconMarkup("ticket")}<span>Нажмите «Добавить мероприятие» и заполните форму.</span></span></p>
                </div>
            `;
            return;
        }
            ...
            <div class="card border-0 shadow-sm p-3 mb-3">
                <div class="d-flex justify-content-between gap-3 flex-wrap">
                    <div>
                        ...
                        <p class="mb-1 meta-with-icon">${getIconMarkup("calendar")}<span>${escapeHtml(eventItem.cityLabel || eventItem.city)} • ${escapeHtml(eventItem.date)} • ${escapeHtml(eventItem.place)}</span></p>
...
async function renderProfile() {
    ...
    info.innerHTML = `
        <div class="card border-0 shadow-sm p-4">
            <h4 class="mb-2 profile-card-line">${getIconMarkup("user")}<span>${escapeHtml(user.name)} ${roleBadge}</span></h4>
            <p class="mb-0 text-muted profile-card-line">${getIconMarkup("login")}<span>${escapeHtml(user.email)}</span></p>
        </div>
    `;
            ...
            ticketsContainer.innerHTML = tickets.map((ticket) => {
                    ...
                    return `
                        <div class="card border-0 shadow-sm p-3 mb-3">
                            <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                                <div>
                                    <h5 class="mb-1 section-title-with-icon">${getIconMarkup("ticket")}<span>Мероприятие недоступно</span></h5>
                ...
                return `
                    <div class="card border-0 shadow-sm p-3 mb-3">
                        <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                            <div>
                                <h5 class="mb-1 section-title-with-icon">${getIconMarkup("ticket")}<span>${escapeHtml(eventItem.title)}</span></h5>
                                <p class="mb-1 text-muted">${escapeHtml(typeLabel(eventItem.type))}</p>
                                <p class="mb-1 meta-with-icon">${getIconMarkup("calendar")}<span>${escapeHtml(eventItem.cityLabel || eventItem.city)} • ${escapeHtml(eventItem.date)} • ${escapeHtml(eventItem.place)}</span></p>
