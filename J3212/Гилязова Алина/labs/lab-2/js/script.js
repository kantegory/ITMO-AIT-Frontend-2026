...
function renderAuthButtons() {
    ...
    if (!user) {
        container.innerHTML = `
            <a href="login.html" class="btn btn-outline-light me-2">Вход</a>
    ...
    container.innerHTML = `
        <a href="profile.html" class="btn btn-outline-light me-2">${escapeHtml(user.name)}</a>
    ...
    return `
        <div class="col-md-4 event-card"
            ...
            <div class="card h-100 event-list-card">
                <img src="${escapeHtml(event.image)}" class="card-img-top" alt="${escapeHtml(event.title)}">
                <div class="card-body d-flex flex-column">
                    <span class="badge text-bg-light align-self-start mb-2">${typeLabel(event.type)}</span>
                    <h5 class="card-title">${escapeHtml(event.title)}</h5>
                    <p class="text-muted mb-3">${escapeHtml(event.cityLabel || event.city)} • ${escapeHtml(event.date)}</p>
                    <a href="event.html?id=${encodeURIComponent(event.id)}" class="btn btn-primary mt-auto">Подробнее</a>
                </div>
            </div>
        </div>
    `;
...
function filterEvents() {
    ...
    cards.forEach((card) => {
        ...
        card.style.display = matchesSearch && matchesType && matchesCity ? "block" : "none";
    });
}
            ...
            <div class="card border-0 shadow-sm p-3 mb-3">
                <div class="d-flex justify-content-between gap-3 flex-wrap">
                    <div>
                        ...
                        <button class="btn btn-danger btn-sm" onclick="deleteCustomEvent('${eventItem.id}')">Удалить</button>
                    </div>
                    ...
                    return `
                        <div class="card border-0 shadow-sm p-3 mb-3">
                            <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                                ...
                                <button class="btn btn-danger" onclick="refundTicket('${ticket.id}')">Вернуть билет</button>
                            </div>
                        </div>
                    `;
                }
