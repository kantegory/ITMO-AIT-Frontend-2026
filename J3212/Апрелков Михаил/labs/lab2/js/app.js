  ...
  const html = routes
    .map((route, index) => {
      ...
      return (
        ...
        '<span class="badge bg-primary-subtle text-primary"><i class="bi bi-buildings me-1"></i> ' +
...
function renderDestination(route, reviews) {
  ...
  if (badges[0]) badges[0].innerHTML = '<i class="bi bi-buildings me-1"></i> ' + (route.type === "nature" ? "Природа" : "Город");
  if (badges[1]) badges[1].innerHTML = '<i class="bi bi-clock me-1"></i> ' + route.durationDays + " дня";
  if (badges[2]) badges[2].innerHTML = '<i class="bi bi-currency-ruble me-1"></i> ~' + Number(route.budget).toLocaleString("ru-RU") + " ₽";
...
function fillProfileHeader(user) {
  ...
  const email = document.querySelector(".text-muted-sm i.bi-envelope");
  if (hello) {
    hello.textContent = "Привет, " + (user.name || "путешественник") + "!";
  }
  if (email && email.parentElement) {
    email.parentElement.innerHTML = '<i class="bi bi-envelope me-1"></i> ' + escapeHtml(user.email);
  }
}
...
function renderCollabActivityLi(a) {
  ...
  return (
    '<li class="list-group-item text-muted-sm"><i class="bi bi-' +
    icon +
    " me-1 text-" +
    color +
    '"></i>' +
